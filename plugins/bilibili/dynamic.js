...
import { purgeLinkInText, request } from './utils.js'
...
const majorFormatters = {
  ...
  MAJOR_TYPE_DRAW: async ({ draw: { items } }) => {
    return items.map(({ src }) => CQ.image(src))
  },
  ...
  MAJOR_TYPE_ARTICLE: ({ article: { covers, id, title, desc } }) => [
    ...(covers.length ? [CQ.image(covers[0])] : []),
    CQ.escape(title.trim()),
    CQ.escape(desc.trim()),
...
export const getDynamicInfo = async id => {
  try {
    const { data } = await request('https://api.bilibili.com/x/polymer/web-dynamic/v1/detail', {
    ...
    msgToConsole(`[error] bilibili get dynamic new info ${id}`)
    console.log(e)
    return null
  }
}
