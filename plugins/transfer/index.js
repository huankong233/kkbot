...
async function transferAccounts(context) {
  ...
  if (from === to) {
    return replyMsg(context, '请不要给自己转账哦~')
  }
  //转账多少只
  const num = parseInt(params[1])
  if (num <= 0) {
    return replyMsg(context, '转账失败,金额有误')
  }
  const to_data = await getUserData(to)
  if (!to_data) {
    return replyMsg(context, '转账失败,转账对象不存在~', true)
  }

  if (!(await reduce(from, num, `转账给${to}`))) {
    replyMsg(context, '转账失败,账户鸽子不足~', true)
  }
  await add(to, num, `转账来自${from}`)
  replyMsg(context, '转账成功~', true)
}
