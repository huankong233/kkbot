...
def compare_image_with_hash(image_file1: str,
                            image_file2: str,
                            max_dif: int = 1.5) -> bool:
    """
    说明:
        比较两张图片的hash值是否相同
    参数:
...
def compressed_image(in_file: Union[str, Path],
    ...
    """
    in_file = in_file if isinstance(in_file, str) else in_file
    if out_file:
        out_file = out_file if isinstance(out_file,
                                          str) else out_file
    else:
        out_file = in_file
    h, w, d = cv2.imread(str(in_file.absolute())).shape
    img = cv2.resize(cv2.imread(str(in_file.absolute())),
...
class BuildImage:
    ...
    def __init__(self,
                 ...
                 color: Union[str, Tuple[int, int, int], Tuple[int, int, int,
                                                               int]] = None,
                 image_mode: str = "RGBA",
                 font_size: int = 10,
        ...
        if not background:
            ...
            if not w and not h:
                ...
                if ratio and ratio > 0 and ratio != 1:
                    self.w = int(ratio * w)
                    self.h = int(ratio * h)
                    self.markImg = self.markImg.resize((self.w, self.h),
                                                       Image.ANTIALIAS)
                else:
                    self.w = w
                    self.h = h
            else:
                self.markImg = Image.open(background).resize((self.w, self.h),
                                                             Image.ANTIALIAS)
        if is_alpha:
            try:
    ...
    async def apaste(
        ...
        alpha: bool = False,
        center_type: Optional[Literal["center", "by_height",
                                      "by_width"]] = None,
    ):
        """
        说明:
    ...
    async def aellipse(
        ...
        参数:
            ...
            :param width: 描线宽度
        """
        await self.loop.run_in_executor(None, self.ellipse, pos, fill, outline,
                                        width)
    ...
    async def atext(self,
        ...
        await self.loop.run_in_executor(None, self.text, pos, text, fill,
                                        center_type, font, font_size, **kwargs)
    ...
    async def arectangle(
        ...
        参数:
            ...
            :param width: 线宽
        """
        await self.loop.run_in_executor(None, self.rectangle, xy, fill,
                                        outline, width)
    ...
    def circle(self):
        ...
        for offset, fill in (width / -2.0, "black"), (width / 2.0, "white"):
            left, top = [(value + offset) * antialias
                         for value in ellipse_box[:2]]
            right, bottom = [(value - offset) * antialias
                             for value in ellipse_box[2:]]
    ...
    async def acircle_corner(
        self,
        radii: int = 30,
        point_list: List[Literal["lt", "rt", "lb",
                                 "rb"]] = ["lt", "rt", "lb", "rb"],
    ):
        """
        说明:
    ...
    async def areplace_color_tran(
        self,
        src_color: Union[Tuple[int, int, int], Tuple[Tuple[int, int, int],
                                                     Tuple[int, int, int]]],
        ...
        """
        self.loop.run_in_executor(None, self.replace_color_tran, src_color,
                                  replace_color)
        ...
        for i in range(self.w):
            for j in range(self.h):
                ...
                if not end_:
                    ...
                    if (start_[0] <= r <= end_[0] and start_[1] <= g <= end_[1]
...
class BuildMat:
    ...
    def __init__(
        ...
        *,
        ...
        if not y_index:
            ...
            _max_value = ceil(_max_value / eval("1" + "0" *
                                                (len(str(_max_value)) - 1))
    ...
    def set_title(self, title: str, color: Optional[Union[str, Tuple[int, int,
                                                                     int]]]):
        """
        说明:
            设置标题
            ...
            if display_num:
                ...
                if is_barh:
                    ...
                    self.markImg.text(
                        (
                            self.padding_w +
                            int(y[i] * self._p * self._deviation) + 2 + 5,
                            current_h - int(font_h / 2) - 1,
                        ),
                        f"{y[i]:.2f}"
                        if isinstance(y[i], float) else f"{y[i]}",
                    )
                else:
                    w = int(self.markImg.getsize(str(y[i]))[0] / 2)
                    self.markImg.text(
    ...
    def _init_graph(
        ...
        for _x in x_index:
            ...
            A.paste(text, (current_w - w, padding_h + line_length + 10),
                    alpha=True)
            current_w += _interval
        ...
        for s in new_text.split("\n"):
            ...
            for _ in range(s.count("[placeholder_")):
                ...
                if "font_size" in placeholder[2]:
                    ...
                    if r:
                        w, h = BuildImage(0, 0, font_size=int(
                            r.group(1))).getsize(placeholder[3])
            ...
            for _ in range(s.count("[placeholder_")):
                if not s.startswith(
                    ...
                    slice_ = s.split(
                        f"[placeholder_{current_placeholder_index}]")
                    await A.atext((current_width, A.h - basic_font_h - 1),
                                  slice_[0], font_color)
                ...
                _img_h = (int(A.h / 2 - text_img.h / 2) if new_text
                          == "[placeholder_0]" else A.h - text_img.h)
                await A.apaste(text_img, (current_width, _img_h - 1), True)
                current_width += text_img.w
                s = s[s.index(f"[placeholder_{current_placeholder_index}]") +
...
def group_image(
        image_list: List[BuildImage]) -> Tuple[List[List[BuildImage]], int]:
    """
    说明:
        根据图片大小进行分组
    参数:
    ...
    is_use.clear()
    while abs(max_h - max_w) > 200 and len(image_group) - 1 >= len(
            image_group[-1]):
