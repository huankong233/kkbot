  ...
  eventReg('message', async (event, context, tags) => {
    if (context.command) {
      if (context.command.name === '装弹') {
        ...
        await feiqiu(context)
      } else if (context.command.name === '欧皇榜') {
        await ouhuang(context)
...
export const createGame = async context => {
  ...
  if (count_data.length === 0) {
    ...
    if (!(await reduce({ user_id, number: pigeon, reason: '参加比赛' }))) {
    ...
    global.russian.wait_id = setInterval(async () => {
      if (global.russian.time === 0) {
        ...
        await add({ user_id, number: pigeon, reason: '比赛超时' })
        clearInterval(global.russian.wait_id)
      }
      global.russian.time--
    }, 1000)

    await replyMsg(context, `回复${global.config.bot.prefix}接受比赛就可以加入比赛啦~`)
  }
}
...
export const acceptGame = async context => {
  ...
  if (count_data.length === 0) {
    ...
    //判断用户能否发起
    if (!(await reduce({ user_id, number: global.russian.addon, reason: '参加比赛' }))) {
      return await replyMsg(context, '你的鸽子不足哦~')
    }
...
export const startGame = async context => {
  ...
  global.russian.wait_id = setInterval(async () => {
    if (global.russian.time === 0) {
      await replyMsg(
        context,
        '超时咯,上一回合射击者获胜~(如果没有人开枪就都返还了哦~而且会扣除手续费)'
      )
      if (global.russian.last_user === 0) {
        let addon = Math.ceil(global.russian.addon * (1 - global.config.russian.clip))
        await add({ user_id: global.russian.from_id, number: addon, reason: '比赛超时' })
        await add({ user_id: global.russian.to_id, number: addon, reason: '比赛超时' })
      } else {
        let addon = Math.ceil(global.russian.addon * 2 * (1 - global.config.russian.clip))
        await add({ user_id: global.russian.last_user, number: addon, reason: '比赛超时' })
      }
      clearInterval(global.russian.wait_id)
      global.russian = {}
    }
...
export const shot = async context => {
  ...
  if (global.russian.mag[global.russian.shot_index] === '*') {
    ...
    let addon = Math.ceil(global.russian.addon * 2 * (1 - global.config.russian.clip))
    if (global.russian.from_id === context.user_id) {
      await add({ user_id: global.russian.to_id, number: addon, reason: '比赛获胜' })
      await replyMsg(
        context,
      ...
      global.russian.end = true
      await add({ user_id: global.russian.from_id, number: addon, reason: '比赛获胜' })
      await replyMsg(
        context,
    ...
    await replyMsg(context, '咔一声，转轮到了下一个位置。')
    global.russian.time = global.config.russian.time
