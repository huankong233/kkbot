...
async function give(context) {
  ...
  if (redPacket_num <= 0 || pigeon_num <= 0) {
    return replyMsg(context, '红包发送失败,红包数量和鸽子数都不能<=0', true)
  }
  //校验合理性
  const pre = pigeon_num / redPacket_num
  if (pre < 1) {
    return replyMsg(context, '红包发送失败,每个包需要至少一只鸽子', true)
  }
  if (parseInt(pre) !== pre) {
    return replyMsg(context, '红包发送失败,每个包里的鸽子数需要为整数', true)
  }
  if (!(await reduce(user_id, pigeon_num, `发送鸽子红包_${code}`))) {
    return replyMsg(context, '红包发送失败,账户鸽子不足', true)
  }
...
async function get(context) {
  ...
  for (let i = 0; i < global.redPacketList.length; i++) {
    ...
    if (picked_user.indexOf(user_id) !== -1) {
      return replyMsg(context, '红包领取过了哦,不要贪心啦~')
    }
    //判断剩余红包数(如果剩余1个,全部拿走)
    ...
    await database
      ...
      .where('id', item.id)
      .from('red_packet')
    replyMsg(context, `红包${item.code}领取成功,获得${get_pigeon_num}只鸽子~`, true)
    //更新红包列表
    await freshRedPacketList()
  }
}
...
async function freshRedPacketList() {
  global.redPacketList = await database
    .select()
    .where('pigeon_num', '!=', 0)
    .from('red_packet')
}
...
function getAll(context) {
  if (global.redPacketList.length !== 0) {
    ...
    global.redPacketList.forEach(item => {
      msg += `\n由${getUserName(item.send_user_id)}发送的口令为:"${item.code}"，剩余${item.pigeon_num
        }只鸽子的红包`
    })
    replyMsg(context, msg.slice(0, -1))
  } else {
    replyMsg(context, '暂时还没有红包哦~要不你发一个?')
  }
}
